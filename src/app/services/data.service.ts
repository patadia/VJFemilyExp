...
export class DataService {
  ...
  ReadMember() {
      return this.database.executeSql('SELECT * FROM TMembersdata where isDelete = ?',[false]).then(data => {
        let members: TMember[] = [];
  ...
  fetchMembers(): Observable<TMember[]> {
    this.ReadMember();
...
//////////////
  ...
  //var start_u =  firebase.default.firestore.Timestamp.fromDate(start);
   //var end_u =  firebase.default.firestore.Timestamp.fromDate(end);
    ...
    return this.database.executeSql('SELECT * FROM TExpensesData where isDelete = ? and Date_unix >= ? and Date_unix <= ? and FamilyKey = ?',[false,start_unix,End_unix,FamilyKey]).then(data => {
      let exp: TExpenes[] = [];
        console.log('readsql',data);
      if (data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
